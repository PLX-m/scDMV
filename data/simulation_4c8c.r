...
setwd("D:/Files/Github/scDMV/")
source("data/functionscDMVm.r")
## 有差异实验
...
for (sim in 1:1000) {
  ...
  qujian<-10
  #这个n1应该是区分4cell和8cell的分水岭
  n1<-48
  # 10 73  每列抽取10个不为NA的数值
...
save(simulation, file="data/simulationdata4c8c_1.Rdata")

cutoff<-10^(-2)
sum(scSMVpvalue<cutoff)

sum((scSMVpvalue<cutoff)&(absd>=0.2))
